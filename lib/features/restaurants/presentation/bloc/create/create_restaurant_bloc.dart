...
class CreateRestaurantBloc
  ...
  CreateRestaurantBloc(this._repository)
    ...
    on<UpdateRestaurantWithImage>(_onUpdateRestaurantWithImage);
  }
  ...
  Future<void> _onUpdateRestaurantWithImage(
    UpdateRestaurantWithImage event,
    Emitter<CreateRestaurantState> emit,
  ) async {
    emit(
      state.copyWith(
    ...
    try {
      ...
      if (event.rating != null) data['rating'] = event.rating;
      if (event.offer != null && event.offer!.isNotEmpty) data['offer'] = event.offer;
      if (event.cgstRate != null) data['cgstRate'] = event.cgstRate;
      if (event.sgstRate != null) data['sgstRate'] = event.sgstRate;
      if (event.serviceCharge != null) data['serviceCharge'] = event.serviceCharge;
